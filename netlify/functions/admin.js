...
exports.handler = async function(event) {
  ...
  // ── CANLI MAÇ KAYDET (upsert) ──
  if (path === '/live/upsert' && method === 'POST') {
    const fid = body.fixture_id;
    if (!fid) return err('fixture_id gerekli');
    // Varsa güncelle
    const existing = await db('GET', 'live_matches', null, '?fixture_id=eq.' + fid + '&select=id');
    if (existing.data && existing.data[0]) {
      await db('PATCH', 'live_matches', body, '?fixture_id=eq.' + fid);
    } else {
      await db('POST', 'live_matches', body, '');
    }
